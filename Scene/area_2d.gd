...
@export var auto_start: bool = true # Opção para começar sozinho ao chegar perto
...
func interact():
	...
	# Se está digitando, completa a digitação imediatamente
	if esta_digitando:
		finalizar_digitacao_imediato()
		return
	
	# Se o diálogo não está ativo, inicia
	if not dialogo_ativo:
		iniciar_dialogo()
	# Se já terminou de digitar a fala atual, avança para a próxima
	else:
		gerenciar_dialogo()
...
func exibir_proxima_fala():
	...
	# CORREÇÃO: Define o texto primeiro, depois anima os caracteres
	ui_texto.text = texto_completo
	ui_texto.visible_characters = 0
	
	# Anima a digitação caractere por caractere
	for i in range(texto_completo.length() + 1):
		# Verifica se deve parar a digitação
		if not esta_digitando or not dialogo_ativo or is_teleporting: 
			return
			
		ui_texto.visible_characters = i
		
		# Toca som para caracteres não-espaço
		if i > 0 and i <= texto_completo.length() and texto_completo[i-1] != " " and audio_player:
			audio_player.pitch_scale = randf_range(0.9, 1.1)
			audio_player.play()
		
		await get_tree().create_timer(velocidade_digitacao).timeout
	
	# Terminou de digitar esta fala
	esta_digitando = false

func finalizar_digitacao_imediato():
	# Para a digitação e mostra todo o texto imediatamente
	esta_digitando = false
